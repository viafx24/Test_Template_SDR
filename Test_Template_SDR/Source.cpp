...
int main(int argc, char** argv)
{
	...
	if (LMS_SetLOFrequency(device, LMS_CH_RX, 0, 96e6) != 0)
		error();

	//Set sample rate to 8 MHz, ask to use 2x oversampling in RF
	//This set sampling rate for all channels
	if (LMS_SetSampleRate(device, 1e6, 2) != 0)
		error();
	...
	if (LMS_SetLPFBW(device, LMS_CH_RX, 0, 1e6) != 0)
		error();
	...
	if (LMS_Calibrate(device, LMS_CH_RX, 0, 1e6, 0) != 0)
		error();
	...
	const int sampleCnt = 200000; //complex samples per buffer
...
#endif
	ofstream myfile("F:\\Documents\\MATLAB\\LimeSDR\\LimeSDR_Mini_MATLAB_From_Github\\example.txt");

	int iteration_2 = 0;
	int samplesRead;
	auto t1 = chrono::high_resolution_clock::now();
	while (chrono::high_resolution_clock::now() - t1 < chrono::seconds(1)) //run for 5 seconds
	{
		//Receive samples
	...
	for (int iteration = 0; iteration < samplesRead; iteration++)
	{
		myfile << buffer[iteration] << "\n";
	}
